...
environmentReady = environment.extractVarsFromEnvironment([
    'PATH',
    'P4CONFIG',
    'o4DIFF',
    'P4IGNORE',
...
function execP4Command(command, options) {
    if(p4[command]) {
        return environmentReady.then(function(p4Env) {
            var defaultOptions = {
                env: p4Env
            };

            return Q.nfcall(p4[command], $.extend({}, defaultOptions, options));
        });
    }
    else {
        throw new Error('unknown p4 method: ' + command);
    }
}
    ...
    getChanges: function getChanges(editor) {
        ...
        if(editor && editor.getPath && editor.getPath()) {
            ...
            .then(function() {
                ...
                return execP4Command('diff', { cwd: openedBufferFilePath, files: [openedBufferFilename] })
                .then(function(result) {
                    console.log(result);
