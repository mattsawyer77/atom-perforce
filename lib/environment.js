...
function getSourceShellEnvCommand(filename) {
    return "'source " + filename + " >/dev/null && env'";
}
...
if(/zsh$/.test(shellValue)) {
    ...
    envCommands = ['~/.bash_profile', '~/.bashrc'].map(getSourceShellEnvCommand);
}
else {
...
module.exports = {
    ...
    extractVarsFromEnvironment: function extractVarsFromEnvironment(requestedVars) {
        function parseEnvOutput(output) {
            ...
            return splitIntoLines(output).reduce(parseLine, {});
        } // parseEnvOutput
        ...
        return envCommands.reduce(function(result, command) {
            ...
            return result.then(function() {
                ...
                .then(function(stdout) {
                    return parseEnvOutput(stdout, requestedVars);
                })
                .catch(function(err) {
