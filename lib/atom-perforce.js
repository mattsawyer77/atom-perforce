...
module.exports = {
        ...
        if(editor && editor.getPath && editor.getPath()) {
            ...
            .then(function(p4Info) {
                if(!p4Info['clientUnknown.']) {
                    ...
                    .catch(function(err) {
                        atom.notifications.addError('Perforce: failed to open for edit', { detail: err, dismissable: true });
                        console.error(err);
                        return false;
                    });
                }
    ...
    add: function add() {
        ...
        if(editor && editor.getPath && editor.getPath()) {
            ...
            .then(function(p4Info) {
                ...
                .catch(function(err) {
                    atom.notifications.addError('Perforce: failed to open for add', { detail: err, dismissable: true });
                        console.error(err);
                        return false;
                    });
                }
    ...
    sync: function sync() {
        ...
        if(editor && editor.getPath()) {
            execCWD = path.dirname(editor.getPath());
        }
        ...
        .catch(function(err) {
            ...
            if(err.message && err.message.indexOf('File(s) up-to-date.') !== -1) {
                ...
                atom.notifications.addError('Perforce: sync failed', { detail: err, dismissable: true });
    ...
    /**
     * execute p4 revert
     * @param {string=} filepath optional filepath or defaults to active buffer path
     ...
     */
    revert: function revert(filepath, confirm) {
        ...
        function executeRevert() {
            ...
            .catch(function(err) {
                atom.notifications.addError('Perforce: revert failed', { detail: err, dismissable: true});
        ...
        if(editor && editor.getPath && editor.getPath()) {
            ...
            atom.notifications.addWarning('Perforce: this file is not currently opened');
            console.warn('no file is currently open to be reverted');
            deferred.resolve();
        }
    ...
    getOpenedFiles: function getOpenedFiles() {
        ...
        if(projectRoots && projectRoots.length) {
            projectRoots.forEach(function(projectRoot) {
                ...
                .catch(function(err) {
                    atom.notifications.addError('Perforce: failed to get opened file list', { detail: err, dismissable: true });
                    console.error(err);
                    deferred.reject(err);
                });
            });
    ...
    markOpenFiles: function markOpenFiles(openedFiles) {
        ...
        .catch(function(err) {
            atom.notifications.addError('Perforce: failed to indicate currently opened files', { detail: err, dismissable: true });
        });
    }
};
